...
class ChannelHelpGenerator extends GeneratorForAnnotation<ChannelHelp> {
  ...
  generateForAnnotatedElement(
    ...
    var channelHelper = Class((builder) {
      ...
      classBuilder.fields.add(Field((fieldBuild) {
        fieldBuild.name = "_${className.toLowerCase()}";
      ...
      if (methodElements != null && methodElements.length > 0) {
        methodElements.forEach((methodElement) {
          classBuilder.methods.add(Method((methodBuild) {
            ...
            methodBuild.returns =
                refer(methodElement.returnType.getDisplayString(withNullability: true));
    ...
    String channelHelperStr = DartFormatter().format('${channelHelper.accept(emitter)}');
  ...
  Code _generatorSingleInstantBody() {
    final blocks = <Code>[];
    blocks.add(Code("_${className.toLowerCase()} ??= ${className}Imp._internal();"));
    blocks.add(Code("return _${className.toLowerCase()};"));
    return Block.of(blocks);
  }

  // ignore: missing_return
  Code _generatorBody(MethodElement methodElement, List<ParameterElement> parameters) {
    final blocks = <Code>[];
    if (parameters == null || parameters.length == 0) {
      blocks
          .add(Code("await $channelName.invokeMethod('${methodElement.name}');"));
    }
